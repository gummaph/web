...
def time_request(called):
    def f(*args,**kwargs):
        ...
        app.logger.info(f"response time: {request.time}")
        return res
    f.__name__=called.__name__
    return f
...
@time_request
def create_employee():
    log_message = {'tracking_id': request.req_id, 'operation': 'create employee', 'status': 'processing'}
...
def update_employee(employee_no):
    log_message = {'tracking_id': request.req_id, 'operation': 'update employee', 'status': 'processing'}
    request.logger.info(str(log_message))
    employee = request.json
    try:
    ...
    return str(company[employee_no]),200
